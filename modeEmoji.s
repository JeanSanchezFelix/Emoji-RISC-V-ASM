...
check_dpad:
    ...
    la t1, pos_y
    lw t2, 0(t1)
    beqz t2, check_down         # already at top, don't move
    addi t2, t2, -1
    sw t2, 0(t1)
    j end_check  
...
check_down:
    ...
    la t1, pos_y
    lw t2, 0(t1)
    addi t3, s2, -16            # max y pos (matrix height - emoji height)
    bge t2, t3, check_left      # already at bottom, don't move
    addi t2, t2, 1
    sw t2, 0(t1)
    j end_check  
...
check_left:
    ...
    la t1, pos_x
    lw t2, 0(t1)
    beqz t2, check_right        # already at left edge, don't move
    addi t2, t2, -1
    sw t2, 0(t1)
    j end_check  
...
check_right:
    ...
    la t1, pos_x
    lw t2, 0(t1)
    addi t3, s1, -16            # max x pos (matrix width - emoji width)
    bge t2, t3, end_check       # already at right edge, don't move
    addi t2, t2, 1
    sw t2, 0(t1)
